        ...
        protected internal void CreateDocProps()
        {
            ...
            using (var subStreamWriter = new StreamWriter(subStream, Encoding.UTF8))
            {
                subStreamWriter.Write("<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" +
                         ...
                         $"<dcterms:created xsi:type=\"dcterms:W3CDTF\">{DateTime.UtcNow:yyyy-MM-ddTHH:mm:ss.FFFZ}</dcterms:created>" +
                         $"<dcterms:modified xsi:type=\"dcterms:W3CDTF\">{DateTime.UtcNow:yyyy-MM-ddTHH:mm:ss.FFFZ}</dcterms:modified>" +
                         "</cp:coreProperties>");
            }
        }
